...
async function getFourSquare(business){
    const options = {
        ...
        let lat = myMap.coordinates[0]
        let lon = myMap.coordinates[1]
        let response = await fetch('https://cors-anywhere.herokuapp.com/https://api.foursquare.com/v3/places/search?query=business&ll=latitude%2C%20longitue&limit=5', options)
        let data = await response.text()
        let parsedData = JSON.parse(data)
        let businesses = parsedData.results
        return businesses
...
// getFourSquare process
    function processBusinesses(data) {
        let businesses = data.map((element) => {
            ...
            long: element.venue.location.longitude,
        };
            return location
        })
        return businesses
